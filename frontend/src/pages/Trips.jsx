...
export default function Trips() {
  ...
  // helper to parse MM/DD/YYYY → Date
  const parseMDY = str => {
    const dt = parse(str, 'MM/dd/yyyy', new Date())
  ...
  const handleSave = async trip => {
    ...
    // Validate activities
    const errors = []
    finalTrip.itinerary.forEach(day => {
      day.activities.forEach((activity, idx) => {
        if (!activity.name.trim()) errors.push(`Activity name is required on ${day.date} (activity ${idx + 1})`)
        if (!activity.time.trim()) errors.push(`Activity time is required on ${day.date} for "${activity.name || 'Unnamed activity'}"`)
      })
    })
    if (errors.length > 0) {
          ...
          <div className="mx-auto" style={{ maxWidth: '500px' }}>
            <TripList
              ...
              onDelete={(id) => {
                ...
                handleDelete(id, trip?.destination)
              }}
            />
          </div>
        </>
