  ...
  const generateFollowupMessage = async (tripData) => {
    ...
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                `You are Trekka, Trip Trek's travel assistant. Give a short, helpful follow up message about their trip. ` +
                `If appropriate, also provide 3 suggestion prompts. ` +
                `Always respond in this JSON format: {"message": "your message text", "suggestions": ["suggestion1", "suggestion2", "suggestion3"]} ` +
                `If no suggestions are appropriate, use an empty array for suggestions.`
            },
            {
              role: 'user',
              content: `I'm planning a trip to ${tripData.destination}. Here's my itinerary: ${JSON.stringify(tripData.itinerary, null, 2)}`
      ...
      try {
        ...
        // Always set suggestions (empty array if none provided)
        setSuggestions(parsed.suggestions || [])

        if (!isOpen) setHasNotification(true)
      } catch (parseError) {
        console.error('Error parsing AI response:', parseError)
        // Fallback: treat the content as a regular message
  ...
  const sendMessage = async (text) => {
    ...
    try {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { 
              role: 'system', 
              content: `You are Trekka, Trip Trek's travel assistant helping users plan trips. ` +
                       `Always respond with a helpful message. If appropriate, also provide suggestions for follow-up questions or actions. ` +
                       `Respond in this JSON format: {"message": "your response text", "suggestions": ["suggestion1", "suggestion2", "suggestion3"]} ` +
                       `If no suggestions are appropriate, use an empty array for suggestions. ` +
                       (tripData ? `The user is planning a trip to ${tripData.destination}. Here's their itinerary:\n` +
                      JSON.stringify(tripData.itinerary, null, 2) : '')
            },
      ...
      if (content) {
        try {
          const parsed = JSON.parse(content)
          const botReply = parsed.message || content
          ...
          const typeNextWord = () => {
            if (i < words.length) {
              ...
              setMessages(prev => [...prev, { sender: 'bot', text: botReply }])
              setStreamedMessage('')
              // Set suggestions after message is complete
  ...
  return (
    <div className="chatbot-wrapper">
      ...
      <div className={`chatbox ${isOpen ? 'open' : ''}`}>
        ...
        {/* Show suggestions whenever they exist */}
